            ...
            <div className="p-6">
                ...
                <div className="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <span className={`inline-block mt-1 px-2 py-1 text-xs font-semibold rounded-full ${user?.role === 'ELEVE' ? 'bg-blue-100 text-blue-800' :
                            user?.role === 'ENSEIGNANT' ? 'bg-purple-100 text-purple-800' :
                                'bg-green-100 text-green-800'
                            }`}>
                            {user?.role.substring(5)}
                        </span>
                    </div>
                    <div className="bg-gray-200 border-4 border-dashed rounded-xl w-28 h-28" />
                </div>
                ...
                {user?.role === 'PARENT' && user?.childEmail && user?.childEmail.length > 0 && (
                    <div className="mb-3">
                        <p className="text-sm text-gray-500">Enfants associés</p>
                ...
                {user?.reActif && user?.reActif.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                        <p className="text-sm text-gray-500 font-medium mb-2">Historique de réactivation</p>
                ...
                {user?.dateNotActif && user?.dateNotActif.length > 0 && (
                    <div className="mt-3">
                        <p className="text-sm text-gray-500 font-medium mb-2">Périodes d'inactivité</p>
                        <ul className="space-y-1">
